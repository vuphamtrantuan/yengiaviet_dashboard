...
**TaskFlow** is a Trello-like task management app (shared Kanban boards → lists →
drag-and-drop cards) with workspace users, filters, and soft archive.
...
- Before running the app for the first time, apply `supabase/schema.sql` in Supabase SQL Editor so all tables/indexes/triggers exist (includes `members.name` and `cards.archived_at`).
- The API routes are server-side only and use service-role credentials via `src/lib/supabase.ts`.
- Boards are **shared workspace-wide** (not per-account). Users are managed via `/api/members` and the `/users` page.
